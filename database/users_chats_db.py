...
class Database:    
    ...
    # In users_chats_db.py, inside the `class Database:`:

    # ... (Keep all existing methods)
    
    async def get_user_data(self, user_id):
        # The user collection is named self.users
        return await self.users.find_one({"id": user_id})

    async def update_file_count(self, user_id):
        today = datetime.date.today()
        user = await self.users.find_one({"id": user_id})

        if not user:
            # Create a basic user entry if not found
            await self.users.update_one(
                {"id": user_id},
                {"$set": {"daily_file_count": 1, "last_file_request_date": today.isoformat()}},
                upsert=True
            )
            return
        
        last_request_date_str = user.get("last_file_request_date")
        
        if last_request_date_str:
            last_request_date = datetime.date.fromisoformat(last_request_date_str)
        else:
        ...
        if last_request_date == today:
            ...
            # New day, reset count and set new date
            await self.users.update_one(
                {"id": user_id},
                {"$set": {"daily_file_count": 1, "last_file_request_date": today.isoformat()}}
            )
    ...
    async def is_premium_user(self, user_id):
        ...
        # Check if expiry_time exists and is in the future
        if expiry_time and expiry_time > datetime.datetime.now(pytz.timezone('Asia/Kolkata')):
             return True
        return False
     ...
     # --- ADD NEW METHODS FOR FILE LIMIT TRACKING ---
    
    async def get_user_file_stats(self, user_id: int):
        """
        Fetches the user's current file count and checks premium status.
        Returns: (current_count: int, limit: int, is_premium: bool)
        """
        today = datetime.datetime.now(pytz.timezone('Asia/Kolkata')).date()
        # Using self.col for main user details
        ...
        file_count = user.get("daily_file_count", 0)
        last_date_str = user.get("last_file_request_date")
        
        if last_date_str:
            last_date = datetime.date.fromisoformat(last_date_str)
            if last_date != today:
                # New day, count resets to 0
                return 0, DAILY_LIMIT, False
                
        # Same day, return current count
        return file_count, DAILY_LIMIT, False

    async def increment_daily_file_count(self, user_id: int):
        """
        Increments the file count for the day or resets it if it's a new day.
        Returns: The new count, or -1 if the user is premium.
        """
        today = datetime.datetime.now(pytz.timezone('Asia/Kolkata')).date()
        ...
        if not user:
            ...
            await self.col.update_one(
                {"id": user_id},
                {"$set": {"daily_file_count": 1, "last_file_request_date": today_str}},
                upsert=True
            )
            return 1
        ...
        if last_date_str and datetime.date.fromisoformat(last_date_str) == today:
