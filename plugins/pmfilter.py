...
async def file_callback_handler(bot, query: CallbackQuery):
    try:
        ...
        # Assuming the format is 'file#<file_id>'
        ident, file_id = query.data.split("#")
        user_id = query.from_user.id
        chat_id = query.message.chat.id
    except ValueError:
        return await query.answer("Invalid file request data.", show_alert=True)

    # 2. 🎚️ কোটা চেক করুন (Quota Check)
    # db.get_user_file_stats অবশ্যই users_chats_db.py তে যুক্ত করতে হবে
    current_count, limit, is_premium = await db.get_user_file_stats(user_id)
    
    if not is_premium and current_count >= limit:
        # লিমিট শেষ, সতর্কতা মেসেজ দেখান এবং স্টপ করুন।
        return await query.answer(
            script.FILE_LIMIT_ALERT, # Script.py তে যোগ করা হয়েছে
            show_alert=True
        )

    # 3. ✅ কাউন্ট বাড়ান (Increment Count)
    # db.increment_daily_file_count অবশ্যই users_chats_db.py তে যুক্ত করতে হবে
    new_count = await db.increment_daily_file_count(user_id)
    
    # 4. ফাইল ডিটেইলস সংগ্রহ করুন
    files = await get_file_details(file_id)
    if not files:
        return await query.answer("File not found or deleted.", show_alert=True)
    file = files[0] 

    # 5. 📝 নতুন ক্যাপশন তৈরি করুন
    file_name = clean_filename(file.file_name) 
    size = get_size(file.file_size)
    
    # ক্যাপশনের জন্য কাউন্ট প্রদর্শন সেট করুন
    if is_premium:
        count_display = "♾️ (Unlimited)"
    else:
        # new_count হলো এই ফাইল রিকোয়েস্টের *পরের* কাউন্ট
        count_display = f"{new_count}/{DAILY_LIMIT}" 
        
    final_caption = script.FILE_CAPTION.format(
        file_name=file_name,
        size=size,
        count=count_display
    )
    ...
    try:
        await bot.send_cached_media(
            ...
            parse_mode=enums.ParseMode.HTML
            # আপনার বাটন/কিবোর্ড থাকলে এখানে যুক্ত করুন
        )
    except Exception as e:
        print(f"Error sending file: {e}") 
        await query.answer("Failed to send file. Please try again later.", show_alert=True)
        return
        
    await query.answer(f"File: {file_name} sent!", show_alert=False)
